...
/**
 * Franja de alerta viva bajo el saludo. Cifras REALES derivadas de la agenda.
 * Clic → baja a "Lo urgente".
 */
export function AlertStrip({ semana, h48 }: { semana: number; h48: number }) {
  const hayCriticos = h48 > 0;
  ...
  return (
    ...
    >
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[color-mix(in_srgb,var(--critical)_22%,transparent)] text-critical">
        {hayCriticos ? <TriangleAlert className="h-5 w-5" strokeWidth={2} /> : <CalendarClock className="h-5 w-5" strokeWidth={2} />}
      </span>

      <p className="min-w-0 flex-1 text-sm font-medium text-foreground sm:text-[15px]">
        <span className="tabular font-semibold text-critical">{semana}</span> en agenda esta semana
        <span className="mx-2 text-muted-foreground">·</span>
        <span className="tabular font-semibold text-critical">{h48}</span> en las próximas 48h
      </p>
